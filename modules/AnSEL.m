...
(* ::Input::Initialization:: *)
FRoute[expr_FEx]/;Head[$GlobalSetup]=!=Symbol:=FRoute[$GlobalSetup,expr];

FUnroute[expr_]/;Head[$GlobalSetup]=!=Symbol:=FUnroute[$GlobalSetup,expr];

FSimplify[expr_FEx]/;Head[$GlobalSetup]=!=Symbol:=FSimplify[$GlobalSetup,expr];
FSimplify[expr_FEx,OptionsPattern[]]/;Head[$GlobalSetup]=!=Symbol:=FSimplify[$GlobalSetup,expr,(Sequence@@Thread[Rule@@{#,OptionValue[FSimplify,#]}]&@Keys[Options[FSimplify]])];
...
indStruct,externalIndices,externalMomenta,kind,
...
If[f==="NotFound",
f=FirstPosition[IsGrassmann[setup,#]&/@subObj[[1]],True][[1]];
...
Flatten[{subObj[[2,f,1]]//.{Plus[a_,b__]:>List[a,b],Times[a_loopMomentum,b__]:>List[a,b]}}],
(*Grab one of the momenta which is a loopMomentum*)
(MatchQ[#,loopMomentum[__]]||MatchQ[-#,loopMomentum[__]])&
][[1]];
...
f=FirstPosition[
Table[
(IsFermion[setup,subObj[[1,i]]]&&MemberQ[makePosIdx/@Flatten[{subObj[[2,i,1]]/.Plus[a_,b__]:>List[a,b]}],
loopMomentum[__]]&&Not@discard[i])
,{i,1,Length[subObj[[1]]]}]
,True][[1]];
...
f=FirstPosition[
Table[
(IsGrassmann[setup,subObj[[1,i]]]&&MemberQ[makePosIdx/@Flatten[{subObj[[2,i,1]]/.Plus[a_,b__]:>List[a,b]}],
loopMomentum[__]]&&Not@discard[i])
,{i,1,Length[subObj[[1]]]}]
,True][[1]];
...
mom=makePosIdx/@Select[Flatten[{subObj[[2,f,1]]/.Plus[a_,b__]:>List[a,b]}],(MatchQ[#,loopMomentum[__]]||MatchQ[-#,loopMomentum[__]])&];
If[sidx>Length[mom],
discard[f]=True;
sidx=0;
Continue[];
...
If[Total@subObj[[2,All,1]]=!=0,Message[FRoute::conservationFail,subObj,ret];Abort[];];
Continue[]
];
...
kind=If[#[[2]],"f",""]&;
ret=ret/.Thread[
loopMomenta->Table[Symbol[$loopMomentumName<>kind[loopMomenta[[idx]]]<>ToString[idx]],{idx,1,Length[loopMomenta]}]
];
...
makeMomentaAlternatives[mom_]:=Module[{idx},
idx=StringSplit[SymbolName[mom],x:NumberString:>ToExpression@x][[2]];
Return[Alternatives[$loopMomentumName<>ToString[idx],$loopMomentumName<>"f"<>ToString[idx]]]
];

FRoute[setup_,expr_FEx]:=Module[{results,ret,idx,subidx},
results=FRoute[setup,#]&/@(List@@expr);
results=GatherBy[results,Length[#["LoopMomenta"]]&];
results=Map[
<|
"Expression"->FEx@@#[[All,Key["Expression"]]],
"ExternalIndices"->#[[1,Key["ExternalIndices"]]],
"LoopMomenta"->makeMomentaAlternatives/@#[[1,Key["LoopMomenta"]]]
|>&,
results];
...
FUnroute[setup_,assoc_Association]/;isRoutedAssociation[assoc]:=FEx@@(FUnroute[setup,#]&/@(List@@assoc));
FUnroute[setup_,term_FEx]:=FUnroute[setup,#]&/@term;
...
(* ::Input::Initialization:: *)
SeparateTermGroups[setup_,expr_FEx]:=Module[{ret=List@@expr,identifierRep,removeFirsts,groupedDiagrams},
...
(* ::Input::Initialization:: *)
SubFSimplify[setup_,expr_FEx]:=Module[
...
SubFSimplify[setup_,expr_FEx,symmetryList_]:=Module[
...
FSimplifyNoSym[setup_,expr_FEx]:=Module[{subGroups,res},
...
FSimplify[setup_,expr_FEx,OptionsPattern[]]:=Module[{subGroups,res,symmetryList},
