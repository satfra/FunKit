...
(* ::Input::Initialization:: *)
MakeSPFormRule[q_,p_,momenta__]:=Module[{momentaList,qf,Defs,nPt,nPtId,nPtCrossId,nPtqId,symmetricPointFiniteTFormRule4pt,repRules,depth=8},
RemoveFromExtraVars/@{momenta};
...
Map[
"id (exMom"<>ToString[#]<>".intMom)^-1=(sqrt(avMom.avMom)*sqrt(intMom.intMom)*cos"<>ToString[#]<>")^-1;\n"<>
"id exMom"<>ToString[#]<>".intMom=sqrt(avMom.avMom)*sqrt(intMom.intMom)*cos"<>ToString[#]<>";\n"&
,Range[1,nPt]
],
Map[
"id (exMom"<>ToString[#]<>".intMomf)^-1=(sqrt(avMom.avMom)*sqrt(intMom.intMom)*cos"<>ToString[#]<>")^-1;\n"<>
...
CreateFormSet[];
ParallelTable[FormTracer`FormTrace[makeHashFile[{a}[[idx]],name<>"/"],{a}[[idx]],preReplRules,postReplRules],{idx,1,Length[{a}]}]
];
...
CreateFormSet[];
ParallelMap[FormTracer`FormTrace[makeHashFile[#],#,preReplRules,postReplRules]&,{a}]
];
