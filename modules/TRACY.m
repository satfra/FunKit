...
MakeSPFormRule[{loopMomenta__}, p_, {momenta__}] :=
    Module[{momentaList, loopMomentaList, Defs, nPt, nLoops, nPtId, nPtCrossId,
         nPtqId, symmetricPointFiniteTFormRule4pt, repRules, depth = 8, i, j},
        ...
        Defs = "Vector " <> StringJoin[Map["exMom" <> ToString[#] <> 
            ","&, Range[1, nPt]]] <> StringJoin[Map["intMom" <> ToString[#] <> ","&,
             Range[1, nLoops]]] <> "avMom;";
        nPtId = StringJoin[Map["id (exMom" <> ToString[#] <> ".exMom"
             <> ToString[#] <> ")^-1=(avMom.avMom)^-1;\n" <> "id exMom" <> ToString[
            #] <> ".exMom" <> ToString[#] <> "=avMom.avMom;\n"&, Range[1, nPt]]];
            
        nPtCrossId = StringJoin[Map["id (exMom" <> ToString[#[[1]]] <>
             ".exMom" <> ToString[#[[2]]] <> ")^-1=(-avMom.avMom/" <> ToString[nPt
             - 1] <> ")^-1;\n" <> "id exMom" <> ToString[#[[1]]] <> ".exMom" <> ToString[
            #[[2]]] <> "=-avMom.avMom/" <> ToString[nPt - 1] <> ";\n"&, Subsets[Range[
            1, nPt], {2}]]];
        nPtqId = StringJoin[Table["id (exMom" <> ToString[i] <> ".intMom"
             <> ToString[j] <> ")^-1=(sqrt(avMom.avMom)*sqrt(intMom" <> ToString[
            j] <> ".intMom" <> ToString[j] <> ")*cos(exMom" <> ToString[i] <> ",intMom"
             <> ToString[j] <> "))^-1;\n" <> "id exMom" <> ToString[i] <> ".intMom"
             <> ToString[j] <> "=sqrt(avMom.avMom)*sqrt(intMom" <> ToString[j] <>
             ".intMom" <> ToString[j] <> ")*cos(exMom" <> ToString[i] <> ",intMom"
             <> ToString[j] <> ");\n", {i, 1, nPt}, {j, 1, nLoops}]];
...
";
        repRules = Thread[Join[Map["exMom" <> ToString[#]&, Range[1, 
            nPt]], Map["intMom" <> ToString[#]&, Range[1, nLoops]], {"avMom"}] ->
             Join[Map[ToString, momentaList], Map[ToString, loopMomentaList], {ToString[
            p]}]];
...
FEx /: FormTracer`FormTrace[name_String, FEx[a__], preReplRules_ : {},
     postReplRules_ : {}] :=
    Module[{idx},
        CreateFormSet[];
        ParallelTable[FormTracer`FormTrace[name <> "/" <> ToString[idx
            ], {a}[[idx]], preReplRules, postReplRules], {idx, 1, Length[{a}]}]
    ];

FTerm /: FormTracer`FormTrace[file_String, FTerm[a__], preReplRules_ 
    : {}, postReplRules_ : {}] :=
...
FEx /: FormTracer`FormTrace[FEx[a__], preReplRules_ : {}, postReplRules_
     : {}] :=
    Module[{},
        CreateFormSet[];
        ParallelMap[FormTracer`FormTrace[makeHashFile[{#, preReplRules,
             postReplRules}], #, preReplRules, postReplRules]&, {a}]
    ];

FTerm /: FormTracer`FormTrace[FTerm[a__], preReplRules_ : {}, postReplRules_
     : {}] :=
    Module[{expr, origVars, tmpfileName, import, repl, formReps, result,
         pref},
        ...
        FormTracer`AddExtraVars @@ GetAllCustomSymbols[expr /. repl[[
            1]]];
        formReps = Map[#[[2]] -> #[[1]]&, FormTracer`GetExtraVarsSynonyms[
            ]];
        FunKitDebug[2, "Custom symbols in expression: ", GetAllCustomSymbols[
            expr /. repl[[1]]]];
        FormTracer`FormTrace[expr /. repl[[1]], preReplRules, Join[$standardFORMmomentumRules,
             postReplRules], {tmpfileName, "O4,saIter=5000,saMinT=10,saMaxT=10000",
             "fortran90"}];
        ...
        result = (ToExpression[import] // Rationalize) /. repl[[2]] /.
             formReps // DiagramSimplify;
        Return[pref * result];
    ];
...
DiagramSimplify[expr_] :=
    Module[{collected, mSimplify, couplings},
        mSimplify = Quiet @ Simplify[Simplify[#, Trig -> False, TimeConstraint
             -> 0.1], Trig -> False, TimeConstraint -> 1]&;
...
FORMSimplify[obj_, preReplRules_ : {}, postReplRules_ : {}] :=
    Module[{file, origVars, tmpfileName, import, repl, expr, newSymbols,
        ...
        FormTracer`FormTrace[Rationalize[expr /. repl[[1]]], Join[momRule,
             ...
             "O4,saIter=5000,saMinT=10,saMaxT=10000", "fortran90"}];
        import = Import[tmpfileName, "Text"];
        RunProcess[$SystemShell, All, "rm " <> tmpfileName];
        import = import // fortranToMathematica;
        FormTracer`DefineExtraVars[origVars];
