...
If[Not @ TensorBasesInstalled[],
    If[ChoiceDialog["TensorBases does not seem to be installed. Do you want to install it?",
...
Get[SelectFirst[Join[{FileNameJoin[{$UserBaseDirectory, "Applications",
...
$DistributedContexts = $DistributedContexts \[Union] {$Context, "FunKit`Private`",
     "TensorBases`", "FormTracer`", "FormTracer`Private`", "TensorBases`",
     "TensorBases`Private`"}
...
FunKitDebugLevel[newLevel_Integer] :=
    Module[{},
...
StyleBox[\"FTerm\",\nFontColor->RGBColor[1, 0.5, 0]]\)\!\(\*
StyleBox[\" \",\nFontColor->RGBColor[1, 0.5, 0]]\)tag, e.g. the Wetterich equation ",
             MaTeX`MaTeX["\\partial_t \\Gamma = \\frac{1}{2}\\,G_{ab}\\partial_t R^{ab}"
            ], " is given by
    \!\(\*
...
As an example, take the generalised flow equation ",
             MaTeX`MaTeX["\\partial_t \\Gamma = -\\dot\\Phi^a\\Gamma_a + \\frac{1}{2}G^{ac}\\Bigg(\\gamma_c^{\\phantom{c}b}\\partial_t  + 2 \\frac{\\delta\\dot{\\Phi}^b}{\\delta\\Phi^c}\\Bigg){R}_{ab}"
