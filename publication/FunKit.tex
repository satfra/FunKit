...
\end{mmaCell}
%
The gauge particle \mathem{A} is a commuting field and has no partner field. In contrast, there are ghosts and anti-ghosts \mathem{cb}, \mathem{c}, hence they are specified as a pair in a nested list.
...
\begin{equation*}
	...
	\put(-140,-83){\includegraphics[width=0.48\linewidth]{Mathematica_Gluon2P_1.png}}
\end{equation*}
\end{minipage}
\end{widetext}
%
\FunKit has automatically simplified the diagrams resulting from the derivation. We can also do the above in a more manual manner by explicitly specifying derivative operators and resolving them. Any functional derivative is specified as an operator acting on everything after it with \mathem{\mmaDef{FDOp}}, and can be resolved using \mathem{\mmaDef{ResolveFDOp}}. Explicitly, we can write the above also as
%
\begin{widetext}
%\vspace{-2pt}
%\hspace{40pt}
...
\begin{equation*}
		...
		\put(-90,-88){\includegraphics[width=1.1\linewidth]{Mathematica_Gluon2P_2.png}}
\end{equation*}
\end{minipage}
\end{widetext}
%
In the above, we have used the shorthand \mathem{\mmaDef{F}[...]}, which is replaced by \mathem{\mmaDef{FEq}[\mmaDef{FTerm}[...]]}. The result of resolving the derivative operators has not been automatically simplified. This can be done using \mathem{\mmaDef{FSimplify}}, i.e.
%
\begin{mmaCell}{Input}
\mmaDef{ResolveFDOp}[
  \mmaDef{ResolveFDOp}[
...
\end{mmaCell}
%
which yields precisely the same output as when using \mathem{\mmaDef{TakeDerivatives}}.
...
\end{aligned}
	...
	\put(-170,-105){\includegraphics[width=1.05\linewidth]{Mathematica_Gluon2P_3.png}}
\end{equation*}
\end{minipage}
\end{widetext}
%
...
\vspace{-3.0ex}
\begin{equation*}
\put(-165,0){\footnotesize\medmuskip=0mu
	$
...
\mmaDef{gluonDSE}["0-Loop"]//\mmaDef{FPrint}
\mmaDef{gluonDSE}["1-Loop"]["LoopMomenta"]
\mmaDef{gluonDSE}["1-Loop"]//\mmaDef{FPrint};
\mmaDef{gluonDSE}["2-Loop"]//\mmaDef{FPlot};
\end{mmaCell}
\vspace{-3.0ex}
\begin{equation*}
...
\put(-209,-90){\includegraphics[width=0.5\linewidth]{Mathematica_Gluon2P_4.png}}
\end{equation*}
\end{minipage}
\end{widetext}
%
In the above example, we see also that the command \mathem{\mmaDef{FPrint}} can be inserted during a chain of operations, as it only prints the \LaTeX\xspace output and then returns its argument unchanged. There are no ghost-loops in the output, as we have not specified the vertex \mathem{{A,A,cb,c}} in the setup -- if we would have added it, two-loop terms with ghost loops would also appear.
