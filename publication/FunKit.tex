6\documentclass[10pt,prd,nofootinbib,superscriptaddress,twocolumn]{revtex4-2}
\pdfoutput=1
...
There is already a number of libraries available for the computer-algebraic processing of functional equations. Among these are \texttt{DoFun} \cite{Huber:2019dkb} and \texttt{QMeS} \cite{Pawlowski:2021tkk}, which concern themselves with the derivation of functional equations from master equations. \texttt{FormTracer} \cite{Cyrol:2016zqb} and the \TensorBases package \cite{Braun:2025gvq}, which builds on \texttt{FormTracer}, provide tools to trace the tensorial structure of the resulting diagrams.
...
\FunKit builds in this regard heavily on \texttt{FormTracer} and \texttt{TensorBases}, and re-implements a large part of the \texttt{QMeS} package while trying to make the syntax more accessible for other, arbitrary master equations.
