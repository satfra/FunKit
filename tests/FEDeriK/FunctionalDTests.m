...
AppendTo[tests, TestCreate[
    FunKit`Private`FunctionalD[testSetup, Phi[i], Phi[j]], 
    \[Gamma][{Phi, Phi}, {-j, i}], 
    TestID -> "Basic functional derivative"]
];

(* AnyField derivative *)
AppendTo[tests, TestCreate[
    FunKit`Private`FunctionalD[testSetup, AnyField[i], Phi[j]], 
    \[Gamma][{Phi, AnyField}, {-j, i}], 
    TestID -> "AnyField functional derivative"]
];

(* Derivative of field without indices (should be zero) *)
AppendTo[tests, TestCreate[
    FunKit`Private`FunctionalD[testSetup, Phi, Phi[j]], 
    0, 
    TestID -> "Derivative of field without indices"]
];
...
(* GammaN derivative *)
AppendTo[tests, TestCreate[
    FunKit`Private`FunctionalD[testSetup, GammaN[{a, b}, {i, j}], Phi[k]], 
    GammaN[{Phi, a, b}, {-k, i, j}], 
    TestID -> "GammaN derivative"]
];
...
AppendTo[tests, TestCreate[
    CheckAbort[FunKit`Private`FunctionalD[testSetup, FEx[a], Phi[i]], "AbortTriggered"], 
    "AbortTriggered", 
    TestID -> "FunctionalD of FEx should abort"]
];
...
AppendTo[tests, TestCreate[
    customResult, 
    customResult, 
    TestID -> "Custom functional derivative rule"]
];
...
AppendTo[tests, TestCreate[
    Head[FunKit`Private`SymmetricDerivative[{Phi, Psi}, {i, j}, {Phi, Psi}, {k, l}]], 
    Times, 
    TestID -> "SymmetricDerivative with matching fields"]
];

(* Test that SymmetricDerivative fails with mismatched field counts *)
AppendTo[tests, TestCreate[
    CheckAbort[FunKit`Private`SymmetricDerivative[{Phi}, {i}, {Phi, Psi}, {j, k}], "AbortTriggered"], 
    "AbortTriggered", 
    TestID -> "SymmetricDerivative with mismatched fields should abort"]
];
...
AppendTo[tests, TestCreate[
    Head[multiIndexResult], 
    Times, 
    TestID -> "Multi-index functional derivative"]
];
...
AppendTo[tests, TestCreate[
    Head[nestedFuncResult], 
    FTerm, 
    TestID -> "Derivative of function applied to FTerm"]
];
...
AppendTo[tests, TestCreate[
    multiFieldResult =!= Undefined, 
    True, 
    TestID -> "Multi-field functional derivative"]
];
...
AppendTo[tests, TestCreate[
    FunKit`Private`FunctionalD[testSetup, 1, Phi[i]], 
    0, 
    TestID -> "Derivative of constant"]
];
...
AppendTo[tests, TestCreate[
    linearResult1, 
    linearResult2, 
    TestID -> "Functional derivative linearity"]
];
