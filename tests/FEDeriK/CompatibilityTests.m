...
AppendTo[tests, VerificationTest[
    FunKit`FunKitForm[testSetup, {DoFun`DoDSERGE`P[Phi[l1], Phi[-l1], Global`explicit -> False]}],
    FEx[FTerm[Propagator[{Phi, Phi}, {{l1}, {-l1}}]]],
    TestID -> "E5a: FunKitForm DoFun algebraic Propagator"
]];

AppendTo[tests, VerificationTest[
    FunKit`FunKitForm[testSetup, {DoFun`DoDSERGE`V[Phi[l1], Phi[-l1], Phi[l2], Phi[-l2], Global`explicit -> False]}],
    FEx[FTerm[GammaN[{Phi, Phi, Phi, Phi}, {{l1}, {-l1}, {l2}, {-l2}}]]],
    TestID -> "E5b: FunKitForm DoFun algebraic V to GammaN"
]];

AppendTo[tests, VerificationTest[
    FunKit`FunKitForm[testSetup, {DoFun`DoDSERGE`S[Phi[-l1], Phi[l1], Global`explicit -> False]}],
...
AppendTo[tests, VerificationTest[
    FunKit`FunKitForm[testSetupYukawa, {DoFun`DoDSERGE`P[Psi[lf1, a1], Psibar[-lf1, a1], Global`explicit -> False]}],
...
AppendTo[tests, VerificationTest[validFExQ[fQmesRT3] && Length[fQmesRT3] === Length[fQmesFK3], True, TestID -> "F1f: QMeS Yukawa vertex: roundtrip produces valid FEx with same diagram count"]];
...
AppendTo[tests, VerificationTest[validFExQ[fQmesFullFK3], True, TestID -> "F4b: QMeS FullDiagrams Yukawa vertex -> FunKitForm produces valid FEx"]];
...
AppendTo[tests, VerificationTest[validFExQ[fQmesFullRT3], True, TestID -> "F4f: QMeS FullDiagrams Yukawa vertex: roundtrip produces valid FEx"]];
