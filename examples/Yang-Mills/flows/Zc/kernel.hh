...
namespace DiFfRG {
  ...
  class Zc_kernel
  {
    ...
    static KOKKOS_FORCEINLINE_FUNCTION auto kernel(const double& l1, const double& cos1, const double& p, const double& k, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& ZA3, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& ZAcbc, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& ZA4, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& dtZc, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& Zc, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& dtZA, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& ZA)
    {
      ...
      const auto _interp4 = ZAcbc(0.816496580927726 * sqrt(powr<2>(l1) - cos1 * l1 * p + powr<2>(p)));
      const auto _interp5 = dtZc(k);
      const auto _interp6 = RB(powr<2>(k), powr<2>(l1));
      const auto _interp7 = RBdot(powr<2>(k), powr<2>(l1));
      const auto _interp8 = Zc(k);
      const auto _interp9 = Zc(1.02 * k);
      const auto _interp10 = Zc(l1);
      const auto _interp11 = dtZA(pow(1. + powr<6>(k),0.16666666666666666667));
      const auto _interp12 = ZA(1.02 * pow(1. + powr<6>(k),0.16666666666666666667));
      const auto _interp13 = ZA(l1);
      const auto _interp14 = Zc(sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
      const auto _den1 = powr<-1>(1. + powr<6>(k));
      const auto _den2 = powr<-2>(_interp6 * _interp8 + _interp10 * powr<2>(l1));
      const auto _den3 = powr<-2>(_interp2 * _interp6 + _interp13 * powr<2>(l1));
      const auto _den4 = powr<-1>(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p));
      const auto _den5 = powr<-1>(_interp1 * _interp8 + _interp14 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
      const auto _den6 = powr<-1>(_interp1 * _interp2 + _interp3 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
      return fma(-3., _den3 * _den5 * powr<2>(_interp4) * (-1. + powr<2>(cos1)) * (_interp2 * _interp7 + _interp6 * (_interp11 + 50. * _den1 * (_interp12 - _interp2) * powr<6>(k))), fma(-3., _den2 * _den4 * _den6 * powr<2>(_interp4) * (_interp5 * _interp6 + _interp7 * _interp8 + _interp6 * (-50. * _interp8 + 50. * _interp9)) * (-1. + powr<2>(cos1)) * powr<2>(l1), 0.));
    }
