                                               ...
                                               const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory> &ZA)
{
  using namespace DiFfRG;
      ...
      const auto _interp7 = ZAcbc(0.816496580927726 * sqrt(powr<2>(l1) - cos1 * l1 * p + powr<2>(p)));
      ...
      return fma(-3.,
                 _cse3 * (_interp1 * _interp2 + (_interp4 + 50. * (-_interp2 + _interp5) * _cse4 * powr<-1>(1. + _cse4)) * _interp3) *
                   powr<-2>(_cse7) * powr<2>(_interp7) * powr<-1>(_interp8 * _interp9 + (_cse1 + _cse8 - 2. * cos1 * l1 * p) * _interp10),
                 fma(-3.,
                     _cse3 * ((_interp14 + 50. * _interp15 - 50. * _interp9) * _interp13 + _interp12 * _interp9) * powr<-1>(_cse7) *
                       powr<2>(_interp11) * powr<-2>(_interp13 * _interp9 + (_cse1 + _cse8 + 2. * cos1 * l1 * p) * _interp16),
                     0.)); // clang-format off
