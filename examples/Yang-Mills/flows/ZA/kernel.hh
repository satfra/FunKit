                                               ...
                                               const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory> &ZA)
{
  using namespace DiFfRG;
      ...
      const auto _cse16 = powr<-2>(_cse15); // clang-format off

  return fma(-1.,
    _cse11 * (7. - _cse1) * _cse16 *
        (_interp3 * _interp4 +
         (_interp1 + 50. * (-_interp4 + _interp5) * _cse10 * powr<-1>(_cse12)) * _interp2) *
        _interp10,
    fma(2.,
        _cse11 * (-1. + _cse1) * _cse2 *
        ...
        fma(-4.,
            powr<-1>(1. + _cse10) *
                ...
                _cse11 *
                (_cse2 * (-8. + 7. * _cse1 + powr<2>(_cse1)) * _cse3 + powr<2>(_cse2) * _cse7 +
                 powr<2>(_cse3) * _cse7 + _cse5 * cos1 * powr<3>(l1) * p +
                 _cse5 * cos1 * l1 * powr<3>(p)) *
    ...
    constant(const double &p, const double &k, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory> &ZA3,
             const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory> &ZAcbc,
             const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory> &ZA4,
             const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory> &dtZc,
             const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory> &Zc,
